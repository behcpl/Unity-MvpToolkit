...
{
    public class FixedVerticalListLayout : ICollectionLayout
    {
        private readonly ListLayoutOptions _layoutOptions;

        private float _width;

        public FixedVerticalListLayout(ListLayoutOptions layoutOptions)
        {
        ...
        {
            float offset = index * (_layoutOptions.ElementSize + _layoutOptions.GapSize) + _layoutOptions.Padding.top;

            return new Rect(_layoutOptions.Padding.left, offset, _width - _layoutOptions.Padding.horizontal, _layoutOptions.ElementSize);
        }
